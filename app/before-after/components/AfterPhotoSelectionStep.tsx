...
import { Upload, X, Image, GalleryVertical } from 'lucide-react';
import { useBeforeAfter } from '@/contexts/BeforeAfterContext';
import { motion } from 'framer-motion';

interface AfterPhotoSelectionStepProps {
  onContinue: () => void;
  onBack: () => void;
}

export default function AfterPhotoSelectionStep({ onContinue, onBack }: AfterPhotoSelectionStepProps) {
  const { afterPhoto, setAfterPhoto } = useBeforeAfter();
  const [selectedPhoto, setSelectedPhoto] = useState<string | null>(afterPhoto);
  const [activeTab, setActiveTab] = useState<'local' | 'website'>('website');
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Update local state when context changes
  useEffect(() => {
    setSelectedPhoto(afterPhoto);
  }, [afterPhoto]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold mb-2">Select After Photo</h2>
        <p className="text-gray-400">Choose a photo to use as your "after" image</p>
      </div>
